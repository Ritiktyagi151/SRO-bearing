...
const Navbar = () => {
    ...
    const fetchMenuData = async () => {
      try {
        ...
        const [navRes, catRes, prodRes] = await Promise.all([
          fetchSafe("http://localhost:5001/api/navbar"),
          fetchSafe("http://localhost:5001/api/categories"),
          fetchSafe("http://localhost:5001/api/products"),
        ]);
        ...
        if (
          ...
          const enabledDropdowns = navRes.settings.dropdowns
            .filter((d) => d.enabled)
            .sort((a, b) => a.order - b.order);

          const formattedCategories = enabledDropdowns.map((drop) => {
            let subcategories = [];
            if (drop.displayMode === "category") {
              subcategories = catRes.categories.map((c) => ({
                label: c.name,
                href: `/products?category=${c.slug}`,
              }));
            } else {
              subcategories = prodRes.products.map((p) => ({
                label: p.name,
        ...
        console.error("Error fetching navbar dynamic menus:", error);
      }
    };
  ...
  return (
    <>
      ...
      {/* QUOTE FORM MODAL (rendered outside header so fixed positioning works reliably) */}
      {isQuoteFormOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto z-60">
            <div className="p-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    className="mt-1 block w-full rounded-md border-gray-200 shadow-sm py-2 px-3 text-sm outline-none focus:ring-2 focus:ring-green-400"
                  />
                  {formErrors.name && (
                ...
                <div>
                  ...
                  {formErrors.product && (
                    <p className="text-xs text-red-500 mt-1">{formErrors.product}</p>
                  )}
                </div>
                ...
                <div className="flex items-center justify-between pt-2">
                  <button
                    ...
                    className="inline-flex items-center px-4 py-2 bg-green-600 text-white rounded-md text-sm font-medium hover:bg-green-700 disabled:opacity-60"
                  >
      ...
      {/* MOBILE MENU SIDEBAR (rendered outside header) */}
      {isMenuOpen && (
          ...
          <div className="relative bg-gray-100 w-full max-w-xs h-full overflow-y-auto shadow-xl">
            ...
            <div className="p-4 border-t border-gray-200 mt-auto">
              ...
              <div className="mt-6 text-xs text-gray-500 text-center">
                <p>© {new Date().getFullYear()} SRO Group. All rights reserved.</p>
              </div>
            </div>
          </div>
        </div>
